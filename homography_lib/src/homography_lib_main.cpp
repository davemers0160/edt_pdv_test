...
//-----------------------------------------------------------------------------
void transform_multi_image_bb(uint32_t N,
    ms_image r_img,
    target_rect r_target,
    ms_image * imgs,
    target_rect * img_targets,
    double* fused_data64_t,
    unsigned char* fused_data8_t,
    unsigned int* img_w,
    unsigned int* img_h,
...
)
{
    ...
    *img_w = r_img.img_w;
    ...
    if (tight_box == true)
    {
        // if the bounding box provided is tight around the object then we can just use this for the transformation
        // otherwise computer it based on the existing bounding box
        ref_h.set_rect(r_target.x, r_target.y, r_target.w, r_target.h);
    }
    else
    {
        // using the supplied bounding box
        ref_h.get_bounding_box(ref_img, ref_img_c, r_img.invert_img);
    }
    ...
    for (idx = 0; idx < N; ++idx)
    {
        if (imgs[idx].use_img)
        {
            ...
            if (tight_box == true)
            {
                // if the bounding box provided is tight around the object then we can just use this for the transformation
                // otherwise computer it based on the existing bounding box
                t_img_h[idx].set_rect(img_targets[idx].x, img_targets[idx].y, img_targets[idx].w, img_targets[idx].h);
            }
            else
            {
                // using the supplied bounding box
                t_img_h[idx].get_bounding_box(tmp_img, tmp_img_c, imgs[idx].invert_img);
            }
